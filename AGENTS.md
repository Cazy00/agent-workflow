...
Default to one coordinator. Independent review uses a separate context and canonical sources, without the implementer's conversation. Preserve task-prefixed checkpoints and an explicit session outcome. When a task's work is merged, remove its record; if another task names it in `prerequisites` or `governing`, set it `Done` instead and remove it with the last record that names it. The pull request is the permanent record. Never infer approval from a task field, Git author, process exit, or branch name. Do not publish using an owner account when worker credentials fail.
