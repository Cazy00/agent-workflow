...
Git freshness now discovers the governing dependency closure at both the recorded and current baselines: profile/config/acceptance definitions, the task's milestone, feature/design/governing paths, prerequisite tasks/contracts, decisions affecting the task/feature/milestone/path, and supersession links. It compares the union, including newly applicable and removed records. Unrelated milestone/decision edits do not alone stale a task. Operational totals/claims belong outside governing milestone records; milestone authority changes still stale affected tasks.
