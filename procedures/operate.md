...
Product changes are new milestones. A project's move to a newer workflow version is a task of its own under an authorised milestone: update the pin in `docs/workflow/config.json`, migrate the records the new schema needs, copy any new scaffold file the project wants from the same release (such as `templates/github/wf-status.yml` into `.github/workflows/`, with the trusted branch filled in), check that CI fetches the full history (`fetch-depth: 0`) that `wf ci` reads, rerun `wf records` and the fixtures, and post the outcome. Both acceptance files are read from the baseline, so a project without them merges `docs/workflow/acceptance.json` and the config that classifies the new paths before the change that adds `tests/acceptance-map.json`. Active milestones stay on their adopted version unless the owner decides otherwise (`maintenance.md`).
