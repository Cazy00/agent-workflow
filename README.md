...
Release **v1.3.0** of the supplied consolidated policy: assisted execution procedures, record templates, a one-command adoption scaffold, required project checks, an owner status view that adopting projects also get as a pinned GitHub issue, support for two or more people who share a project with their own agents, and a zero-dependency Node.js 22 validator. Reviewed in an independent context and approved by the owner before tagging.
...
Releases are tags: `v1.0.0` (the version PrintFlow runs), `v1.1.0`, `v1.2.0` and `v1.3.0`. Review and owner approval precede every tag, and a project adopts a release only by changing its own pin; a new tag never upgrades a project by itself.
