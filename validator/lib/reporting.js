...
export function publicReport(input,config) {
  ...
  const e = {...input,id:input.id ?? randomUUID(),occurrences:input.occurrences ?? 1,security:input.security ?? false};
...
export function githubTransport(token,fetcher=fetch) {
  ...
  const request=async(method,route,body)=>{
    ...
    if(!response.ok) {
      ...
      const retryAt=retry && /^\d+$/.test(retry)?Date.now()+Number(retry)*1000:reset && /^\d+$/.test(reset)?Number(reset)*1000:Date.now()+60000;
  ...
  return {
    ...
    list: (destination,worker,page)=>request('GET',`/repos/${destination}/issues?state=all&creator=${encodeURIComponent(worker)}&per_page=100&page=${page}`),
...
export async function deliverReport(state,r,config,transport,save,now=Date.now()) {
  ...
  const used=Object.values(state.reports).filter(x=>x.event.session_id===r.event.session_id && x.event.id!==r.event.id && (x.attempts>0 || x.status==='delivered')).length;
  ...
  try {
    ...
    for(let page=1;page<=3;page++) {
      const list=await transport.list(config.destination,config.worker,page);
    ...
    r.status='unknown';r.attempts++;save();
    phase='create';
