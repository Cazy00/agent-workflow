...
Trusted Git integration requires a committed candidate containing the current baseline; `--base`, if given, must equal the authoritative baseline rather than a historical merge base. `--candidate`/`--head` determines both the records read and the actual diff. `--changed` and directory sources are diagnostic fixtures and cannot replace an authoritative diff when trust evidence is supplied. CLI Git failures exit 2 instead of becoming an empty passing diff. An added record whose path the trusted branch's first-parent history held with a different title, question, outcome or rule fails, since IDs are never reused; the same record restored passes with a note, and a shallow clone exits 2.

`status` is a derived view: it loads the records at the given revision, previews each open task's readiness as if the baseline were approved, and lists open decisions, verified or accepted milestones, blocked tasks, inbox and setup items; with two or more `owners`, what waits on each person. Given `--pull-requests FILE` (`gh pr list` JSON) it maps same-repository pull requests to tasks by the CLI's branch rule or a title starting with the task ID, shows claims and awaited reviews, and flags a task claimed twice, an `Active` task with none and a pull request naming two tasks. It never reads receipts, never checks approval, and exits 0 whenever the records load. `templates/github/wf-status.yml`, installed by the scaffold, publishes it with the open pull requests to a *Project status* issue; it runs only on the trusted branch, never on pull requests, and may write issues and nothing else.
